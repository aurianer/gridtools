 ...
 */
/**
   @file This file shows an implementation of the "horizontal diffusion" stencil, 
 * using the HPX backend
*/
...
int hpx_main_(gt::uint_t d1, gt::uint_t d2, gt::uint_t d3) {
    constexpr unsigned halo = 2;

    // Add dimensions and halos to the storage builder
    auto storage_builder = gt::storage::builder<gridtools::stencil::cpu_ifirst_hpx>.dimensions(d1, d2, d3).halos(halo, halo, 0);
    ...
    // the data fields
    st::run(spec, gridtools::stencil::cpu_ifirst_hpx(), grid, coeff, in, out);
    return hpx::finalize();
}
...
int main(int argc, char **argv) {
    ...
    hpx::util::function_nonser<int(int, char**)> main_function =
        hpx::util::bind(hpx_main_, d1, d2, d3);
    return hpx::init(main_function, argc, argv);
}
